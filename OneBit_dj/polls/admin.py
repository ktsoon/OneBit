...
def image_preview(self): # используется в класе
    if self.img:
        return mark_safe('<img src="{0}" width="150" height="150" />'.format(self.get_image_url('thumbnail')))
    else:
        return '(No image)'

class ImgTovar_Inline(admin.TabularInline): # вывод изображения в ТАБЛИЦЕ "Картинки к товару"
    """ Картинки к товару """
...
class TovarsAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'category', 'avtor', 'cost', 'skidka_cost', 't_count')
    list_filter = ('category', 'avtor')
    list_editable = ['cost', 'skidka_cost']
    prepopulated_fields = { "slug": ("name",)}
    search_fields = ('name','slug')
    inlines = [ImgTovar_Inline, characteristic_Inline]
...
class ImgTovarAdmin(admin.ModelAdmin):
    ...
    list_display = ('tovar','get_img')
    search_fields = ('tovar',)
    readonly_fields = (image_preview,)

    def get_img(self, obj): # вывод изображения в ФОРМЕ "Картинки к товару"
        return mark_safe(f"<img src='{obj.get_image_url('thumbnail')}' width=60 height=60 >")
...
class commentsAdmin(admin.ModelAdmin):
    ...
    list_display = ('user', 'tovar', 'star', 'update_at', 'created_at')
    search_fields = ('tovar', 'user')
